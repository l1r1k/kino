...
9. Собрать и запустить локально redis
   ...
   9.4. Проверить, запустился ли redis, через команду src/redis-cli ping, в ответ в терминале должен быть выведен PONG
...
10. Выдать права доступа на запуск ffmpeg и ffprobe
   ...
   10.3. Проверить работу ffmpeg через ffmpeg/ffmpeg-7.0.2-amd64-static/ffmpeg -version, в терминале должно будет вывеститься информация о ffmpeg
...
12. Запустить django через gunicorn. nohup gunicorn kinoafisha.wsgi:application --bind unix:/var/www/kinoafisha/gunicorn.sock --log-level info --access-logfile /var/www/kinoafisha/logs/access.log --error-logfile /var/www/kinoafisha/logs/error.log &

13. Настроить nginx на обработку запросов на сайт киноафиши
